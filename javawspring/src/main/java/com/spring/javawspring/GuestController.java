...
public class GuestController {
	...
	/*// 페이지네이션 처리전
	 * @RequestMapping(value = "/guestList", method=RequestMethod.GET) public String
	 * guestListGet(Model model,
	 * 
	 * @RequestParam(name="pag", defaultValue = "1", required = false) int pag) { //
	 * 1.페이지(pag)를 결정한다. int pageSize = 3; // 2. 한 페이지의 분량을 결정한다. int totRecCnt =
	 * guestService.totRecCnt(); // 3. 총 레코드 건수를 구한다. int totPage = (totRecCnt %
	 * pageSize)==0 ? totRecCnt / pageSize : (totRecCnt / pageSize) + 1; // 4. 총 페이지
	 * 건수를 구한다. int startIndexNo = (pag - 1) * pageSize; // 5. 현재페이지의 시작 인덱스번호를 구한다.
	 * int curScrStartNo = totRecCnt - startIndexNo; // 6. 현재 화면에 보여주는 시작번호를 구한다.
	 * 
	 * int blockSize = 3; // 1. 블록의 크기를 결정한다.(여기선 3으로 지정) int curBlock = (pag - 1) /
	 * blockSize; // 2. 현재페이지가 위치하고 있는 블록 번호를 구한다.(예:1페이지는 0블록, 3페이지는 0블록, 5페이지는
	 * 1블록) int lastBlock = (totPage - 1) / blockSize; // 3. 마지막블록을 구한다.
	 * 
	 ...
	 * model.addAttribute("totPage", totPage); model.addAttribute("curScrStartNo",
	 * curScrStartNo); model.addAttribute("blockSize", blockSize);
...
//	public String guestSearch(String search,String searchString) {
//		guestService.searchGuest(String search,String searchString);
//		
//		return "guest/
//	}

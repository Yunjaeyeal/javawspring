...
public class MemberController {
	...
	// 전체리스트와 검색리스트를 하나의 메소드로 처리.. ---> 동적쿼리 페이지네이션이용함
	...
	/*
	 * // 전체리스트와 검색리스트를 하나의 메소드로 처리.. ---> 동적쿼리 페이지네이션처리전
	 * 
	 * @RequestMapping(value="/memberList", method=RequestMethod.GET) public String
	 * memberListGet(Model model,
	 * 
	 * @RequestParam(name="mid", defaultValue = "" , required = false ) String mid,
	 * 
	 * @RequestParam(name="pag", defaultValue = "1" , required = false ) int pag,
	 * 
	 * @RequestParam(name="pageSize", defaultValue = "3", required = false) int
	 * pageSize) {
	 * 
	 * int totRecCnt = memberService.totTermRecCnt(mid); int totPage = (totRecCnt %
	 ...
	 * = (totPage - 1) / blockSize;
	 * 
	 * ArrayList<MemberVO> vos = memberService.getMemberList(mid,startIndexNo,
	 * pageSize);
	 * 
	 * model.addAttribute("vos", vos); model.addAttribute("mid",mid);
	 * model.addAttribute("pag", pag); model.addAttribute("totPage", totPage);
	 * model.addAttribute("curScrStartNo", curScrStartNo);
	 * model.addAttribute("blockSize", blockSize); model.addAttribute("curBlock",
	 * curBlock); model.addAttribute("lastBlock", lastBlock);
	 ...
	 * String memberListPost(Model model, String mid,
	 * 
	 * @RequestParam(name="pag", defaultValue = "1" , required = false ) int pag,
	 * 
	 * @RequestParam(name="pageSize", defaultValue = "3", required = false) int
	 * pageSize) {
	 * 
	 * int totRecCnt = memberService.totRecCnt(); int totPage = (totRecCnt %
	 * pageSize)==0 ? totRecCnt / pageSize : (totRecCnt / pageSize) + 1; int
	 * startIndexNo = (pag - 1) * pageSize; // int curScrStartNo = totRecCnt -
	 * startIndexNo;
	 * 
	 ...
	 * memberService.getMemberList(mid,startIndexNo,pageSize);
	 * 
	 * model.addAttribute("vos", vos); model.addAttribute("membersearch",mid);
	 * model.addAttribute("pag", pag); model.addAttribute("totPage", totPage);
	 * model.addAttribute("curScrStartNo", curScrStartNo);
	 * model.addAttribute("blockSize", blockSize); model.addAttribute("curBlock",
	 * curBlock); model.addAttribute("lastBlock", lastBlock); return
	 * "member/memberList"; }
	 */
	@RequestMapping(value = "/memberInfor", method=RequestMethod.GET)
	public String memberInforGet(String mid,HttpSession session,Model model,int pag) {
		
		MemberVO vo = memberService.memberInfor(mid);
